...
# Extract real trades for displaying in the trading ledger
optimal_threshold = 2.0
optimal_strategy = f'Pairs_Z{optimal_threshold}'
optimal_trades = backtest_results[optimal_strategy]['Trades']

# Calculate trade metrics for all strategies
trade_metrics = {}
for threshold in thresholds:
    ...
    if trades:
        winning_trades = sum(1 for trade in trades if trade['Total_Return'] > 0)
        avg_return = sum(trade['Total_Return'] for trade in trades) / len(trades)
        avg_duration = sum(trade['Duration'] for trade in trades) / len(trades)

        trade_metrics[strategy] = {
            'Count': len(trades),
            'Win Rate': winning_trades / len(trades),
...
# Get the optimal strategy's data
optimal_strategy = f'Pairs_Z{2.0}'
optimal_data = backtest_results[optimal_strategy]['Data']

# Create cumulative returns data for plotting
cum_returns_data = pd.DataFrame()

# Add the optimal pairs trading strategy
cum_returns_data[optimal_strategy] = optimal_data['Cumulative_Return']
...
# Calculate drawdown for pairs strategy
pairs_drawdown = backtest_results[optimal_strategy]['Data']['Drawdown']
...
drawdown_data = pd.DataFrame({
    ...
    optimal_strategy: pairs_drawdown,
...
fig = px.line(drawdown_data, x='Date', y=[optimal_strategy, 'Hold_MSTR', 'Hold_BTC'],
...
# Create DataFrame for trade metrics
trade_metrics_df = pd.DataFrame(trade_metrics).T

# Format for display
trade_metrics_df['Win Rate'] = trade_metrics_df['Win Rate'].map('{:.2%}'.format)
trade_metrics_df['Avg Return'] = trade_metrics_df['Avg Return'].map('{:.2%}'.format)
trade_metrics_df['Avg Duration'] = trade_metrics_df['Avg Duration'].map('{:.1f} days'.format)
...
# Get position data for optimal strategy
optimal_strategy = f'Pairs_Z{2.0}'
position_data = backtest_results[optimal_strategy]['Data'][['Position', 'Z_Score', 'MSTR', 'BTC']]
...
# Add threshold lines
fig.add_hline(y=optimal_threshold, line_dash="dash", line_color="red", annotation_text=f"Upper Threshold ({optimal_threshold})")
fig.add_hline(y=-optimal_threshold, line_dash="dash", line_color="red", annotation_text=f"Lower Threshold (-{optimal_threshold})")
fig.add_hline(y=0, line_dash="solid", line_color="black", opacity=0.3)
...
# Layout setup
fig.update_layout(
    title=f"Z-Score and Trading Positions (Threshold = {optimal_threshold})",
    xaxis_title="Date",
    yaxis_title="Z-Score",
    hovermode="x unified",
    height=500
...
# Get trade returns for optimal strategy
optimal_strategy = f'Pairs_Z{2.0}'
trade_returns = [trade['Total_Return'] for trade in backtest_results[optimal_strategy]['Trades']]

# Create histogram of trade returns
fig = px.histogram(
    x=trade_returns,
    nbins=20,
    title=f"Distribution of Trade Returns (Z-Score Threshold = {optimal_threshold})",
...
fig.add_vline(x=median_return, line_dash="dash", line_color="red",
             annotation_text=f"Median: {median_return:.2%}")
...
optimal_vs_hold = pd.DataFrame({
    optimal_strategy: backtest_results[optimal_strategy]['Data']['Cumulative_Return'],
...
fig = px.line(
    optimal_vs_hold,
    x='date',
    y=[optimal_strategy, 'Hold_MSTR', 'Hold_BTC'],
    title=f'Optimal Pairs Strategy (Z={optimal_threshold}) vs Buy-and-Hold',
    labels={
        'value': 'Cumulative Return (Initial Investment = 1)',
        'variable': 'Strategy'
    },
...
While our backtest shows that simply holding MSTR would have yielded higher absolute returns, the pairs trading approach demonstrates significantly better risk characteristics:

1. **Lower Maximum Drawdown** for pairs trading vs. holding MSTR
2. **Lower Volatility** for pairs trading vs. holding MSTR
3. **Comparable Sharpe Ratio** for pairs trading vs. holding MSTR
